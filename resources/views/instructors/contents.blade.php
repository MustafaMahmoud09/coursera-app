    ...
    <section class="contents">
        ...
        <div class="box-container">
            ...
            <div class="box" style="text-align: center;">
                ...
                <a href="{{ route('instructor.add.playlist.content.view') }}" class="btn">add content</a>
            </div>
            ...
            @if (count($contents) != 0)
                ...
                @foreach ($contents as $content)
                    <div class="box">
                        <div class="flex">
                            <!--add this style if status is disactive--> <!--color:red-->
                            <div><i class="fas fa-dot-circle" style="{{ provideStatusColor($content->status) }}"></i><span
                                    style="{{ provideStatusColor($content->status) }}">{{ provideStatusName($content->status) }}</span>
                            </div>
                            <div><i class="fas fa-calendar"></i><span>{{ formatDate($content->created_at) }}</span></div>
                        </div>
                        <img src="{{ asset('storage/'.$content->cover_path) }}" class="thumb" alt="">
                        <h3 class="title">
                            {{ $content->title }}
                        </h3>
                        <form action="{{ route('instructor.delete.playlist.content', $content->id) }}" method="post" class="flex-btn">
                            @csrf
                            @method('delete')
                        ...
                        <a href="{{ route('instructor.playlist.content.details.view', $content->id) }}" class="btn">view content</a>
                    </div>
                @endforeach
            @else
