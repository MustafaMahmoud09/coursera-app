        ...
        <div class="box-container">
            ...
            @if (count($playlists) != 0)
                @foreach ($playlists as $playlist)
                    <div class="box">
                        <div class="flex">
                            <div><i class="fas fa-circle-dot" style="{{ provideStatusColor($playlist->status) }}"></i><span
                                    style="{{ provideStatusColor($playlist->status) }}">{{ provideStatusName($playlist->status) }}</span>
                            </div>
                            <div><i class="fas fa-calendar"></i><span>{{ formatDate($playlist->created_at) }}</span></div>
                        </div>
                        <div class="thumb">
